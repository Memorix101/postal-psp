...
bool ValidateFrame(Uint32* frameBuffer, int width, int height) {
    ...
    for (int i = 0; i < width * height; i++) {
        ...
        if (r == 0 && g == 0 && b == 0 && a == 255) { // Example: fully black pixels
            invalidPixelCount++;
        }
...
{
	static int frameCounter = 0; // Keeps track of the current frame
    const int frameDropRate = 2; // Drop every 3rd frame

    // Increment the frame counter
    frameCounter++;

    // Check if the current frame should be skipped
    if (frameCounter % frameDropRate == 0) {
        // Skip this frame
        return;
    }
