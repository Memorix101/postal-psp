...
typedef struct
{
	...
	uint32_t *data; // Pointer to texture pixel data (VRAM)
} Texture;
...
#if defined(SWITCH) || defined(PSP)
	fprintf(stderr, ">> Create SDL_RENDERER_SOFTWARE %s\n", SDL_GetError());
...
#define SCREEN_HEIGHT BUFFER_HEIGHT
	...
	TexturePointer = new uint32_t[512 * 512];
	if (!TexturePointer)
	{
		fprintf(stderr, "Error: Unable to allocate VRAM for texture.\n");
		exit(1);
	}

	PalettedTexturePointer = new uint8_t[FramebufferWidth * FramebufferHeight];
	if (!PalettedTexturePointer)
	{
		fprintf(stderr, "Error: Unable to allocate memory for palette data.\n");
		exit(1);
	}

	// init stuff
	memset(TexturePointer, 0, FramebufferWidth * FramebufferHeight * sizeof(uint32_t));
	memset(PalettedTexturePointer, 0, FramebufferWidth * FramebufferHeight * sizeof(uint8_t));
	fprintf(stderr, "Texture and palette data successfully initialized.\n");

#endif
	sdlTexture = SDL_CreateTexture(sdlRenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, 512, 512);
	if (!sdlTexture)
	{
		char buf[128];
...
bool ValidateFrame(Uint32 *frameBuffer, int width, int height)
{
	// Simple validation logic (customize based on your distortion patterns)
	int invalidPixelCount = 0;
	const int threshold = (width * height) / 10; // Example: 10% of pixels can be invalid

	for (int i = 0; i < width * height; i++)
	{
		Uint32 pixel = frameBuffer[i];

		// Check for corruption (e.g., all pixels are the same or too many black pixels)
		Uint8 r, g, b, a;
		SDL_GetRGBA(pixel, SDL_AllocFormat(SDL_PIXELFORMAT_ARGB8888), &r, &g, &b, &a);

		if (r == 0 && g == 0 && b == 0 && a == 255)
		{ // Example: fully black pixels
			invalidPixelCount++;
		}

		if (invalidPixelCount > threshold)
		{
			return false; // Frame is distorted
		}
	}
...
typedef struct {
    ...
    unsigned int color;  // Vertex color
...
// Bottom-right corner
vertices[1].u = (float)480;   // Use scaled width
vertices[1].v = (float)272;   // Use scaled height
...
sceGuTexImage(0, 512, 272, 512, TexturePointer);
sceGuTexWrap(GU_CLAMP, GU_CLAMP);
sceGuTexScale((float)480 / 512.0f, (float)272 / 512.0f);  // Scale UVs correctly
...
for (int y = 0; y < FramebufferHeight; y++) {
    for (int x = 0; x < FramebufferWidth; x++, src++, dst++) {
        ArgbColor color = apeApp[*src];
        Uint8 r = color.r;
        Uint8 g = color.g;
        Uint8 b = color.b;
        Uint8 a = color.a;

        *dst = (a << 24) | (b << 16) | (g << 8) | r; // BGRA
    }
}*/
    ...
    Uint32 *dst = TexturePointer;  
    int scaledWidth = 480;
    int scaledHeight = 272;

    for (int y = 0; y < scaledHeight; y++) {
        int srcY = (y * FramebufferHeight) / scaledHeight;
        const Uint8 *srcRow = src + (srcY * FramebufferWidth);

        for (int x = 0; x < scaledWidth; x++, dst++) {
            int srcX = (x * FramebufferWidth) / scaledWidth;
            Uint8 paletteIndex = srcRow[srcX];
            ArgbColor color = apeApp[paletteIndex];
            *dst = (color.a << 24) | (color.b << 16) | (color.g << 8) | color.r;
        }
        dst += (512 - scaledWidth);  // Align to 512 width
    }



fprintf(stderr, "FramebufferWidth: %d, FramebufferHeight: %d\n", FramebufferWidth, FramebufferHeight);
if (!TexturePointer || !apeApp) {
    fprintf(stderr, "Error: TexturePointer or apeApp is NULL.\n");
    return;
}
...
sceGuCopyImage(GU_PSM_8888, 0, 0, 480, 272, 512, TexturePointer, 0, 0, 512, (void*)(((unsigned int)framebuffer)+0x4000000));
sceGuTexSync();
...
//sceKernelDelayThread(16667);
	...
	/*
		...
		int dstWidth = 480, dstHeight = 272;

		if (!frameSurface) {
			frameSurface = SDL_CreateRGBSurfaceFrom(
				TexturePointer, srcWidth, srcHeight, 32,
				FramebufferWidth * 4, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000
			);
			if (!frameSurface) {
		...
		if (!scaledSurface) {
			scaledSurface = SDL_CreateRGBSurfaceWithFormat(0, dstWidth, dstHeight, 32, SDL_PIXELFORMAT_ARGB8888);
			if (!scaledSurface) {
				fprintf(stderr, "Failed to create scaled SDL_Surface: %s\n", SDL_GetError());
		...
		if (SDL_BlitScaled(frameSurface, &srcRect, scaledSurface, &dstRect) != 0) {
			fprintf(stderr, "Failed to scale SDL_Surface: %s\n", SDL_GetError());
			return;
		}

		SDL_Texture* frameTexture = SDL_CreateTextureFromSurface(sdlRenderer, scaledSurface);
		if (!frameTexture) {
